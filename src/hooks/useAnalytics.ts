// ============================================================
// NetForge — Lightweight Analytics Hook
// Tracks user events in localStorage for local usage insights.
// ============================================================

import { getLocalDateString } from "../utils/localDate";

const ANALYTICS_KEY = "netforge_analytics";
...
function loadEvents(): AnalyticsEvent[] {
  try {
    ...
    if (!Array.isArray(parsed)) {
      console.warn("[NetForge Analytics] Corrupted data (non-array) — resetting.");
...
function pushEvent(event: AnalyticsEvent) {
  try {
    ...
    console.error("[NetForge Analytics] Failed to save event:", err);
  }
}
