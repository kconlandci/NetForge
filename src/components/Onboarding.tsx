...
const ONBOARDING_KEY = "netforge_onboarding_complete";
...
const screens: OnboardingScreen[] = [
  {
    ...
    icon: (
      <div className="w-20 h-20 rounded-2xl bg-emerald-500/20 flex items-center justify-center mb-4">
        <Router className="text-emerald-400" size={40} />
      </div>
    ),
  },
  {
    ...
    icon: (
      <div className="flex items-center gap-4 mb-4">
        ...
        <div className="w-14 h-14 rounded-xl bg-slate-800 flex items-center justify-center">
          <Hand className="text-emerald-400" size={24} />
        </div>
        <div className="text-slate-500 text-lg">&rarr;</div>
        <div className="w-14 h-14 rounded-xl bg-slate-800 flex items-center justify-center">
  ...
  },
  {
    ...
    body: "NetForge is for educational purposes only. Scenarios are simulated and designed to build network engineering judgment. Begin with a free beginner lab — no account needed.",
    icon: null,
    isFinal: true,
      ...
      {screen.requiresConsent && (
        <label className="flex items-center gap-3 mb-4 max-w-xs cursor-pointer">
          <input
            ...
            className="w-5 h-5 rounded accent-emerald-500 shrink-0"
          />
